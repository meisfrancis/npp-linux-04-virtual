...
echo "For ingress (if port 80) - from enp0s8 port 80 (web) destination NAT (192.168.56.100 to 10.10.10.1)"
iptables -t nat -A PREROUTING -i enp0s8 -d 192.168.56.100 -p tcp --dport 80 -j DNAT --to-destination 10.10.10.1

echo "For ingress (if port 80) - from enp0s8 port 25565 (minecraft) destination NAT (192.168.56.100 to 11.11.11.1)"
iptables -t nat -A PREROUTING -i enp0s8 -d 192.168.56.100 -p tcp --dport 25565 -j DNAT --to-destination 11.11.11.1

#sudo iptables -t nat -L -v -n


echo "For egress - from vethA source NAT (10.10.10.1 to 192.168.56.100)"
iptables -t nat -A POSTROUTING -o enp0s8 -s 10.10.10.1 -p tcp --sport 80 -j SNAT --to-source 192.168.56.100

echo "For egress - from vethC source NAT (11.11.11.1 to 192.168.56.100)"
iptables -t nat -A POSTROUTING -o enp0s8 -s 11.11.11.1 -p tcp --sport 25565 -j SNAT --to-source 192.168.56.100
...
on node 2 run:
 echo -n "HELLO ns1" | nc 192.168.56.100 80
 echo -n "HELLO ns2" | nc 192.168.56.100 25565
